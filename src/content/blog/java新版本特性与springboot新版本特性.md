...
# Java 8 到 Java 24 主要更新特点与 Spring Boot 新版特点

## 引言：驾驭 Java 和 Spring Boot 的快速演进
...
## 第二部分：Spring Boot 演进 (Spring Boot 2 到 Spring Boot 4)

### Spring Boot 3：云原生与性能焦点

Spring Boot 3 是一个重要的版本，它将 Spring Boot 推向了云原生应用开发的前沿，并带来了显著的性能改进。

#### GraalVM 原生镜像支持

**目的：** 将 Spring Boot 应用转换为可在没有 JVM 的情况下运行的独立原生可执行文件 [5, 7, 9, 13, 14, 65, 66]。

**优势：**

- **即时启动：** 原生镜像几乎可以立即启动，消除了 JVM 预热时间，这对于无服务器函数和微服务至关重要 [13, 14, 65, 66]。
- **更低的内存使用：** 没有运行时编译开销，显著减少内存占用，非常适合容器化环境 [13, 14, 65, 66]。
- **更小的攻击面：** 原生镜像不包含不必要的组件，增强了安全性 [14]。
- **更小的 Docker 镜像大小：** 显著减小了部署包的体积 [13]。

**缺点：**

- **更长的构建时间：** 原生镜像编译通常比 JIT 编译需要更长的时间 [65, 66]。
- **无运行时优化：** 失去 JIT 编译器在运行时进行的自适应优化 [66]。
- **兼容性问题：** 依赖反射或动态代码加载的 Java 特性可能需要额外配置 [14, 66]。
- **调试复杂性：** 传统 JVM 工具无法用于原生镜像的调试和监控 [14]。
...
- **Maven：** `./mvnw -Pnative spring-boot:build-image` 或 `./mvnw -Pnative native:compile` [67]
- **Gradle：** `./gradlew bootBuildImage` 或 `./gradlew nativeCompile` [67]

**运行原生可执行文件：**

- **Maven：** `./target/demo`
- **Gradle：** `./build/native/nativeCompile/demo` 或 `./gradlew nativeRun` [67]

**启动时间比较：**

| 指标 | 之前 (JVM) [13] | 之后 (原生 + 虚拟线程) [13] | | :---------------- | :----------------- | :----------------------------- | | **冷启动时间** | ~3.4 秒 | ~75 毫秒 | | **内存使用 (空闲)** | ~220MB | ~48MB | | **Docker 镜像大小** | ~250MB | ~80MB |

GraalVM 原生镜像支持是 Spring Boot 应用在云原生和无服务器环境中具有革命性意义的功能。它直接解决了 JVM 应用常见的“冷启动”问题和高内存占用，使 Spring Boot 在某些微服务工作负载方面与 Go 或 Rust 等语言具有竞争力。其权衡在于构建时间和动态功能限制。原生镜像通过“提前编译”（AOT）在构建时将 Java 字节码转换为原生二进制文件，消除了运行时 JIT 编译的需要 [14, 66]。这种“封闭世界假设”意味着所有运行时可能调用的字节码和应用依赖项都必须在构建时已知 [14]。这使得编译器只能将可达方法包含在原生可执行文件中，从而导致启动更快、内存使用更少。

#### Java 17 基线

Spring Boot 3.0 要求 JDK 17 或更高版本作为基线 [1, 7, 8, 14, 68]。

**影响：**

- **模块系统：** 对于从 Java 8 运行的应用，主要挑战包括 Java 9 中引入的模块系统以及核心 JDK 中移除的许多 J2EE `javax` 依赖项 [1]。
- **API 弃用与移除：** 自 Java 11 采用六个月发布周期以来，用户必须处理最初无害但随后从 Java 平台中移除的弃用 API [1]。
- **第三方依赖项更新：** 使用 Java EE 规范的应用需要添加显式依赖项，并且许多第三方依赖项可能需要升级以兼容 Java 模块系统 [1]。

Java 17 基线强制许多仍在 Java 8 或 11 上运行的遗留 Spring Boot 2 应用进行重大升级。这不仅仅是版本升级，而是一个根本性转变，需要仔细的依赖管理和代码修复，原因在于 Java 的模块系统和 API 移除。

#### Jakarta EE 9/10 迁移

Spring Boot 3.x 更新之前需要处理的一项重大任务是迁移到 Jakarta EE 9 。

**影响：**

- **命名空间更改：** Java EE 库的包命名空间从 `javax.*` 更改为 `jakarta.*` [1, 5, 7, 9, 14, 69]。这意味着任何引用这些库的代码都必须将其包迁移到新的命名空间 [1, 69]。
- **第三方库兼容性：** Jackson、Hibernate、Tomcat 和 Jetty 等第三方库提供了与 Jakarta EE 9 兼容的替代工件，需要仔细审查以确保使用兼容版本 [1, 5, 14, 69]。
...
**差异：** 迁移到 Jakarta EE 9/10 主要是包命名空间的更改，这要求更新所有相关的导入语句和第三方库依赖。

这对于使用 Java EE API 的任何企业应用来说都是一个重大的破坏性更改。这是一项非简单的迁移工作，会影响应用的几乎每个层面，突显了对自动化重构工具的需求。

#### 使用 Micrometer 进行可观测性

Spring Boot 3 中对可观测性的支持已达到通用可用性，它基于 Micrometer Observation 构建，涵盖日志、指标和跟踪三个核心支柱 [5, 7, 9, 13, 14, 70, 71]。

**主要特性：**

- **统一 API：** Micrometer Observation 提供了一个统一的 API，用于生成指标和跟踪 [70]。
- **上下文传播：** 依赖上下文传播库在线程和响应式管道中转发当前观测数据 [70]。
- **通用标签：** 支持为所有观测数据应用低基数通用标签，用于环境维度钻取 [70]。
- **OpenTelemetry 支持：** 提供了基本的 OpenTelemetry 支持，并与 Micrometer Tracing 结合使用时自动配置跟踪 [7, 9, 13, 70, 71]。
- **注解支持：** 支持 `@Observed`、`@Timed`、`@Counted` 等 Micrometer 观测注解 [70]。

**优势：**

- **全面的数据收集：** 通过统一 API 捕获跟踪、指标和日志 [71]。
- **自动仪表化：** 对 Spring MVC 控制器、Spring Data 仓库等进行自动仪表化，减少手动配置 [71]。
- **简化监控：** 提供了对应用内部状态的深入洞察，简化了故障排除和性能分析 [5]。
...
public void doSomething() {
    Observation.createNotStarted(&quot;doSomething&quot;, this.observationRegistry)
           .lowCardinalityKeyValue(&quot;locale&quot;, &quot;en-US&quot;) // 低基数标签
           .highCardinalityKeyValue(&quot;userId&quot;, &quot;42&quot;) // 高基数标签（仅用于跟踪）
           .observe(() -&gt; {
                // 在此处执行业务逻辑
                System.out.println(&quot;Executing business logic with observation.&quot;);
            });
}
}
```
...
Spring Boot 3.2 及更高版本提供了对 Java 21 及更高版本中虚拟线程的内置支持

**优势：**

- **简化并发：** 允许应用以“每请求一线程”的编程风格处理更多请求，而无需复杂的异步编程模型 [1, 5]。
- **提高可伸缩性：** 通过在阻塞 I/O 操作时卸载底层平台线程，虚拟线程可以显著提高应用的吞吐量和可伸缩性 [1, 5]。
- **易于启用：** 只需设置一个属性 `spring.threads.virtual.enabled=true` 即可启用虚拟线程 [5]。

Spring Boot 对 Java 21 虚拟线程的无缝集成是开发人员轻松采用这种革命性并发模型的关键推动因素，进一步提高了可伸缩性并简化了 I/O 密集型应用开发，而无需进行重大代码更改。

### 迁移中的挑战

- **类加载器更改：** 应用类加载器现在是内部类，不再是 `URLClassLoader` 的实例。这可能影响依赖于旧类加载器行为的应用 [5]。
- **Apple 特定包的替换：** 某些 Apple 特定包（如 `com.apple.eawt`）已被平台无关的 `java.awt.Desktop` 包中的对应项取代 [5]。
- **第三方 JAR 更新：** 迁移需要将第三方 JAR 更新到最新版本，例如 Hibernate 6.3、Jackson 2.15、Tomcat 10.1 等，以确保兼容性并利用最新功能 [5]。
- **配置属性更改：** 某些配置属性已被弃用或更改，例如 `server.max.http.header.size` 已被 `server.max-http-request-header-size` 取代 [5]。
- **Trailing Slash 匹配：** Spring Boot 3 弃用了配置 trailing slash 匹配的选项，如果需要，必须手动定义配置类 [5]。
- **模块系统和 API 移除：** 对于从 Java 8 迁移的应用，Java 9 引入的模块系统以及 Java 11 之后移除的 API 可能会带来挑战，需要添加显式依赖或重构代码 [1]。
- **`javax` 到 `jakarta` 命名空间更改：** 这是 Spring Boot 3 迁移中的一项重大任务，涉及将所有 Java EE 库的包名从 `javax.*` 更改为 `jakarta.*`，并更新所有相关的第三方库

### 现代化工具和策略

为了应对这些挑战，存在多种工具和策略可以帮助简化迁移过程：

- **`Jdeprscan` 工具：** Java 提供了 `Jdeprscan` 工具，用于识别代码中使用的弃用或已移除的 API

- **`spring-boot-properties-migrator`：** 此工具可以处理 Spring Boot 3 中大多数必需的配置属性更改

- **OpenRewrite：** 这是一个强大的自动化重构工具，可以帮助从 Spring Boot 2.x 版本迁移到最新版本，并处理 Java 升级和 Jakarta EE 迁移等任务

- **迁移分析报告工具：** Java 21 提供了分析现有应用并生成详细迁移规划报告的工具，识别需要修改的具体行号，并突出强制性和推荐更改
